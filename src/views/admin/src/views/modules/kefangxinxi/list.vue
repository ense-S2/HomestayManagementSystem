<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          ...
          <el-form-item>
            <!-- 查询按钮绑定了 @click="search()" 事件。见第846行 -->
            <el-button
              v-if="
          ...
          >
            <template slot-scope="scope">
              <div v-if="scope.row.kefangtupian">
                <img
                  :src="$base.url + scope.row.kefangtupian.split(',')[0]"
                  width="400"
                  height="350"
                />
              </div>
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                ...
                size="mini"
                @click="
                  dingfangxinxiCrossAddOrUpdateHandler(
                    scope.row,
                    'cross',
                    '',
                    '',
                ...
                "
                >{{ contents.tableBtnFont == 1 ? "订房" : "" }}</el-button
              >
              <el-button
                v-if="
                  isAuth('kefangxinxi', '订房') &&
                ...
                "
                >{{ contents.tableBtnFont == 1 ? "订房" : ""
                }}<i class="el-icon-tickets el-icon--right"
              /></el-button>
              <el-button
                v-if="
                  isAuth('kefangxinxi', '订房') && contents.tableBtnIcon == 0
                "
                type="success"
                size="mini"
                @click="
    ...
    <dingfangxinxi-cross-add-or-update
      v-if="dingfangxinxiCrossAddOrUpdateFlag"
      :parent="this"
      ref="dingfangxinxiCrossaddOrUpdate"
    ></dingfangxinxi-cross-add-or-update>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      // 发起HTTP GET请求获取分页数据
      this.$http({
        url: "kefangxinxi/page", // 请求的API路径
        method: "get", // 请求方法
        params: params, // 请求参数
      }).then(({ data }) => {
        // 如果请求成功且返回的状态码为0（表示成功）
        if (data && data.code === 0) {
          // 更新数据列表和总条数
          this.dataList = data.data.list;
          this.totalPage = data.data.total;
        } else {
          // 如果请求失败，清空数据列表并设置总条数为0
          this.dataList = [];
          this.totalPage = 0;
        }
