<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('kefangxinxi','订房') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingfangxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'订房':'' }}</el-button>
                <el-button v-if="isAuth('kefangxinxi','订房') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingfangxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'订房':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('kefangxinxi','订房') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingfangxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'订房':'' }}</el-button>
    ...
    <dingfangxinxi-cross-add-or-update v-if="dingfangxinxiCrossAddOrUpdateFlag" :parent="this" ref="dingfangxinxiCrossaddOrUpdate"></dingfangxinxi-cross-add-or-update>
...
import dingfangxinxiCrossAddOrUpdate from "../dingfangxinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dingfangxinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    dingfangxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dingfangxinxiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.dingfangxinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.dingfangxinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
